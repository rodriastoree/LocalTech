...
import { BookOpen, Check, Package, ShoppingCart } from 'lucide-react';
...
const Suppliers = lazy(() =>
  import('./views/Secondary').then((m) => ({ default: m.Suppliers })),
);
const Help = lazy(() =>
  import('./views/Secondary').then((m) => ({ default: m.Help })),
);
const Settings = lazy(() =>
  import('./views/Secondary').then((m) => ({ default: m.Settings })),
);
function Onboarding() {
  const [step, setStep] = useState(() => {
    try {
      return localStorage.getItem('localtech-onboarding') ? 4 : 0;
    } catch {
      return 0;
    }
  });
  ...
  const data = [
    {
      icon: BookOpen,
      title: 'Todo lo importante, a la vista',
      text: 'LocalTech muestra ventas, stock y alertas con palabras simples.',
    },
    {
      icon: ShoppingCart,
      title: 'Vendé en pocos pasos',
      text: 'Buscá el producto, agregalo y elegí cómo pagó el cliente.',
    },
    {
      icon: Package,
      title: 'El stock se actualiza solo',
      text: 'Cada venta descuenta unidades y te avisa cuándo conviene reponer.',
    },
  ][step];
  const finish = () => {
    try {
      localStorage.setItem('localtech-onboarding', 'done');
    } catch {
...
function Routed() {
  return (
    <AppShell>
      <Suspense
        fallback={
          <div className="grid min-h-96 place-items-center text-sm font-bold text-slate-500">
            Cargando…
          </div>
        }
      >
        <Routes>
          ...
          <Route path="/nueva-venta" element={<POS />} />
